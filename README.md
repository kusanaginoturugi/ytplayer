...
再生開始時には、手動予約がなければタイトル検索の2番目を仮の次曲としてバックグラウンドでダウンロードする。手動で次曲を予約すると仮予約は破棄・置換される。準備済みの次曲は同じ `mpv` プレイリストへ追加し、ギャップレス再生する。0.5秒以上の無音は再生時に取り除く（長い無音を含む動画は曲中の無音も短くなる）。
